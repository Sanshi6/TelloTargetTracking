...
class Tello:
    ...
    def __init__(self, local_ip, local_port, imperial=False, command_timeout=.3, tello_ip='192.168.10.1',
        ...
        # man operation
        self.operation_thread = threading.Thread(target=self._operation)
        self.operation_thread.daemon = True
        self.operation_thread.start()

        # life command
        self.operation_thread = threading.Thread(target=self._sent_life)
        self.operation_thread.daemon = True
        self.operation_thread.start()

        # anto operation
        # self.operation_thread = threading.Thread(target=self.Control_Logic)
        # self.operation_thread.daemon = True
        # self.operation_thread.start()

        # self._video_frame_queue = queue.Queue(128)
            ...
            try:
                ...
                if len(res_string) != 1460:
                    for frame in self._h264_decode(packet_data):
                        self.frame = frame
                    # try:
                    #     self._video_frame_queue.put(self.frame)
                    # except Exception as e:
                    #     print("queue is full, drop one frame.")
    ...
    def Control_Logic(self, arg):
        a, b, c, d = 0, 0, 0, 0
        if arg[0] - self.Area_ori[0] > self.Area_ori[0] * 0.1:
            b = -20
        elif self.Area_ori[0] - arg[0] > self.Area_ori[0] * 0.1:
            b = 20
        else:
            b = 0

        if arg[1] - self.Area_ori[1] > 96:
            d = 20
        elif self.Area_ori[1] - arg[1] > 96:
            d = -20
        else:
            d = 0

        if arg[2] - self.Area_ori[2] > 72:
            c = -20
        elif arg[2] - self.Area_ori[2] < -72:
            c = 20
        else:
            c = 0

        # Boundary error prevention
        top_y = arg[1] - arg[3] / 2         # The y value in the upper left corner
        low_y = top_y + arg[1]              # The y value in the lower right corner
        low_x = arg[0] - arg[2] / 2         # The x value in the upper left corner
        top_x = low_x + arg[2]              # The y value in the lower right corner
        ...
        self.socket.sendto("rc {0} {1} {2} {3}".format(a, b, c, d).encode('utf-8'), self.tello_address)
        print("rc {0} {1} {2} {3}".format(a, b, c, d))
